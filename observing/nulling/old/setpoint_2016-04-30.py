...
n_success_confirm = 2 # Number of successful setpoint confirmations required
# ====================================================================================================
...
setpoint_current = np.copy(setpoint_old) # This parameter is used to keep track of the CURRENT setpoint during all offsets sent by the script.
...
if (src == 'UBC') and (not pi.getINDI("PLC.CloseLoop.Yes")): # Initial check if phase loop closed (can only be done on UBC)
  if raw_input("PHASE LOOP OPEN! Please close loop and press [ENTER] to continue or [c] + [ENTER] to abort. ") == "c": quit()
...
#    print setpoints
    ...
    setpoint_current = np.copy(setpoints[i]) # go final step ...
    pi.setINDI('PLC.PLSetpoint.PLSetpoint=' + str(setpoint_current) + ';forNAC=' + forNAC)
...
#        if np.abs(null[j]) < 0.001:
#          if raw_input("ROIs stuck, please move ROIs and press [ENTER] to continue or [c] + [ENTER] to abort. ") == "c": quit()
