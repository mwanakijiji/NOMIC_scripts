...
save_data = 1         # Save data during setpoint optimization? 1 for Yes, 0 for No
# ====================================================================================================
...
#setpoint0 = 0.0     # Use this for testing (will always start at setpoint 0 and avoids walking off too far when running the script multiple times on noise only.
...
while n_success < 5:
  ...
  for i in range(0, n_points):
    ...
    # Set new setpoint
    if src == 'UBC':
      pi.setINDI('PLC.PLSetpoint.PLSetpoint=' + str(setpoints[i]) + ';forNAC=0')
    else: # if src == 'NAC'
      pi.setINDI('PLC.PLSetpoint.PLSetpoint=' + str(setpoints[i]) + ';forNAC=1')
...
if src == 'UBC':
  pi.setINDI('PLC.PLSetpoint.PLSetpoint=' + str(np.average(setpoints_fit)) + ';forNAC=0')
else: # if src == 'NAC'
  pi.setINDI('PLC.PLSetpoint.PLSetpoint=' + str(np.average(setpoints_fit)) + ';forNAC=1')
