...
n_phot  = 200          # number of photometric frames (default is 200)
n_bckg  = n_phot       # number of background frames at the end (default is n_phot)
...
settings  = pi.getINDI('PLC.UBCSettings.*', wait=True)
beam2_y   = settings["PLC.UBCSettings.Beam2_y"]
...
for j in range(n_nod):
	...
	pi.setINDI("NOMIC.Command.text", "0 autodispwhat %i obssequences" % (n_null), wait=True)
	...
	settings['PLC.UBCSettings.Beam2_y'] = beam2_y + offset_b2
	pi.setINDI(settings) 
	...
	if j != n_nod-1:
		...
		# Move beam 2 position
		settings['PLC.UBCSettings.Beam2_y'] = beam2_y
		pi.setINDI(settings)
...
if skip_phot < 1:
	...
	# Take background
	pi.setINDI("NOMIC.Command.text", "rawbg 1 autodispwhat 0 savedata 1 contacq", timeout=50000, wait=True)
	
	#Now pause to recover AO
	wait4AOrunning()
	
	#Turn off contacq, save data, and turn off display
	pi.setINDI("NOMIC.Command.text", "0 autodispwhat %i savedata 0 contacq %i obssequences" % (save, n_phot), timeout=50000, wait=True)
	...
	#Turn off contacq, save data, and turn off display
	pi.setINDI("NOMIC.Command.text", "0 contacq 0 autodispwhat %i savedata %i obssequences" % (save, n_bckg), wait=True)
	time.sleep(sleep)			
	pi.setINDI("NOMIC.Command.text", "go", timeout=50000, wait=True)	
