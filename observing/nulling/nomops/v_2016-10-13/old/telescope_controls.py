...
def nod(cfg, nod_dir, move_tel=True, side='both'):
  ...
  if (move_tel == True) and (side == 'both'):
    print('INFO: Nodding the telescope. Configuration parameters:')
  elif (move_tel == True) and (side != 'both'):
    print('INFO: Nodding the %s side only. Configuration parameters:' % (side))
  else:
    print('INFO: Doing a "nod" on PHASECAM and NOMIC only. Configuration parameters:')
...
def offset_background(cfg, off_dir):
  ...
  print('')
  print('INFO: Offsetting the telescope. Configuration parameters:')
